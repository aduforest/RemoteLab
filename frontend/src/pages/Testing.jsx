  ...
  useEffect(() => {
    if (duts.length > 0 && Object.keys(dutLinks).length > 0) {
        ...
        const svg = d3.select("#dutMap")
            ...
            .attr("width", width)
            .attr("height", height)
            .style("filter", "drop-shadow(0px 4px 10px rgba(0, 0, 0, 0.1))"); // Drop shadow
        ...
        gradient.append("stop")
            .attr("offset", "0%")
            .attr("stop-color", "#E8BCEF") // Lighter shade
            .attr("stop-opacity", 1);

        gradient.append("stop")
            .attr("offset", "100%")
            .attr("stop-color", "white") // Original color
            .attr("stop-opacity", 1);
      ...
      const simulation = d3.forceSimulation(duts)
      ...
      const links = svg.append("g")
        .selectAll("line")
        .data(Object.values(dutLinks).flat())
        .enter().append("line")
        .attr("stroke", "black");
      ...
      simulation.nodes(duts).on("tick", ticked);
      simulation.force("link", d3.forceLink().id(d => d.id).links(Object.values(dutLinks).flat()))
    ...
    <main className="bg-purple px-6 pt-16 pb-24 lg:px-8 relative">
      ...
      <div className="mx-auto max-w-6xl py-10 sm:py-14 lg:py-18 z-10 relative">
        <div className="text-center">
          <h1 className="text-4xl tracking-tight text-gray-900 sm:text-5xl mt-10 mb-10">
            {reservation.name}
          </h1>
        </div>
